...
func ScanTo(s interface{}, d interface{}) error {
	...
	if s == nil {
		return fmt.Errorf(`tox.ScanTo, s is nil`)
	}

	// validate to d is pointer of struct
	if d == nil {
		return fmt.Errorf(`tox.ScanTo, d is nil`)
	}
	if reflect.TypeOf(d).Kind() != reflect.Ptr {
		return fmt.Errorf(`tox.ScanTo, d is not pointer`)
	}
	vd := reflect.ValueOf(d)
	if vd.IsNil() {
		return fmt.Errorf(`tox.ScanTo, value of d is nil`)
	}
	...
	if vdk != reflect.Struct && vdk != reflect.Slice {
		return fmt.Errorf(`tox.ScanTo, type of d is not struct or slice`)
	}
...
func setValue(field reflect.Value, v interface{}) error {
	...
	case reflect.Array:
		if field.Type() == reflect.TypeOf((*uuid.UUID)(nil)).Elem() {
			...
			return fmt.Errorf(`kind of pointer not in case-4[%v]`, ftk)
		}
	...
	case reflect.Ptr:
		...
		case reflect.Array:
			if field.Type().Elem() == reflect.TypeOf((*uuid.UUID)(nil)).Elem() {
				...
				return fmt.Errorf(`kind of reflect.Array not in case-3[%v]`, field.Type().Elem().Kind())
			}
		...
		case reflect.Struct:
			if field.Type().Elem() == reflect.TypeOf((*time.Time)(nil)).Elem() {
				...
				return fmt.Errorf(`kind of reflect.Struct not in case-4[%v]`, field.Type().Elem())
			}

		default:
			return fmt.Errorf(`kind of reflect.Ptr not in case-2[%v]`, field.Type().Elem().Kind())
		}
	default:
		return fmt.Errorf(`kind of field not in case-1[%v]`, ftk)
	}
